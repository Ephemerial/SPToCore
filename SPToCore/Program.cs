...
namespace SPToCore
{
    static class Program
    {
        ...
        public static string P_Context;
        public static string P_Schema;
        public static string P_RoutineName;
        ...
        public static void SPToCoreScan() {
            ...
            SPToCoreT4 spToCoreT4Processed = new SPToCoreT4(SpList, P_NameSpace,P_OutPutSolutionFolder, P_Context);
        ...
        private static Command SPToCoreScanCommand()
        {
            ...
            cmd.AddOption(new Option(new[] { "--routine", "-rtn" }, "SQL SERVER routine name (partial like)")
            {
                Argument = new Argument<string>()
                {
                    Arity = ArgumentArity.ExactlyOne
            ...
            cmd.Handler = CommandHandler.Create<string,string,string,string,string,string,string>((cnn, nsp,sch, rtn, ctx,sf, pf) => {
                ...
                using (SqlConnection connection = new SqlConnection(P_ConnectionString))
                {
                    string sql = $@"SELECT * FROM INFORMATION_SCHEMA.ROUTINES
                                 WHERE ROUTINE_TYPE = 'PROCEDURE' AND LEFT(ROUTINE_NAME, 3) NOT IN ('sp_', 'xp_', 'ms_')
                                          AND ROUTINE_SCHEMA = '{P_Schema}' and ROUTINE_NAME LIKE '%{P_RoutineName}%' ORDER BY ROUTINE_NAME";
